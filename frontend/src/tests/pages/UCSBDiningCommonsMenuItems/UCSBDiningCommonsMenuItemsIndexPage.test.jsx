...
describe("UCSBDiningCommonsMenuItemsIndexPage tests", () => {
  ...
  test("renders three Items correctly for regular user", async () => {
    ...
    expect(screen.queryByText(/Create UCSBDiningCommonsMenuItems/)).not.toBeInTheDocument();
  });
  ...
  test("renders empty table when backend unavailable, user only", async () => {
    ...
    expect(errorMessage).toMatch(
      "Error communicating with backend via GET on /api/ucsbdiningcommons/all",
    );
    restoreConsole();
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    // assert
    await waitFor(() => {
      expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItems with id 1 was deleted");
    });
  });
});
